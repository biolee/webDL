...
namespace webdl {
   ...
   */
  function forwardProp(network: Node[][], inputs: number[]): number {
   ...
   * in the network.
   */
  function backProp(network: Node[][], target: number,
    errorFunc: ErrorFunction): void {
    // The output node is a special case. We use the user-defined error
    // function for the derivative.
    let outputNode = network[network.length - 1][0];
    outputNode.outputDer = errorFunc.der(outputNode.output, target);
    //outputDer是这个节点的error
   ...
   * derivatives.
   */
  function updateWeights(network: Node[][], learningRate: number,
  ...
  function forEachNode(network: Node[][], ignoreInputs: boolean,
  ...
  function getOutputNode(network: Node[][]) {
